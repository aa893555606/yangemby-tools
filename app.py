    ...
    if policy.get('IsDisabled') != should_disable:
        ...
        try:
            ...
            if r.status_code in [200, 204]:
                # --- 推送逻辑 ---
                status_text = "🚫 账号已封禁" if should_disable else "✅ 账号已激活/延期"
                ...
                send_bark_msg("yangemby 状态变更", f"用户: {uname}\n状态: {status_text}\n到期日: {target_date}")
                return True
            return False
        except: return False
    return True
    ...
    with st.container(border=True):
                    ...
                    if st.session_state.cfg.get('auto_ban_popcorn') and is_streaming:
                        if any(kw in (client + device) for kw in ["爆米花", "popcorn", "Popcorn"]):
                            if not is_disabled:
                                ...
                                is_disabled = True
                                # --- 推送逻辑 ---
                                send_bark_msg("🚨 违规行为封禁", f"用户: {uname}\n原因: 使用违规播放器 ({client})")
        ...
        with c3:
            if is_forever:
                ...
                delta = (d - today).days
                # ===== 修改这里：当天显示剩余 1 天 =====
                display_days = delta + 1 if delta >= 0 else -delta
                if delta >= 0:
                    st.markdown(f'<span class="pill pill-green">剩余 {display_days} 天</span>', unsafe_allow_html=True)
                else:
                    st.markdown(f'<span class="pill pill-red">已过期 {display_days} 天</span>', unsafe_allow_html=True)
